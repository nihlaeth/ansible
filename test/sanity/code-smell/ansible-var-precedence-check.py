...
class VarTestMaker(object):
    ...
    def write_playbook(self):
        ...
        if self.roles:
            ...
            for role in self.roles:
                role.write_role()
                pb_copy['roles'].append(role.name)
        if self.tasks:
    ...
    def build(self):
        ...
        if 'include_vars' in self.features:
            self.tasks.append(dict(include_vars='included.yml'))
...
def main():
    ...
    while features:
        ...
        #print('%s - %s' % (res, fdesc[res]))
        print("FEATURE: %-30s EXPECTED: %-30s (%s)" % (features[0], res, fdesc[features[0]]))
